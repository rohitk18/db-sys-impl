...
int Schema :: Find (char *attName) {
	...
	int j = 0;
	while(attName[j]!='.' && attName[j]!=NULL)
	{
		j++;
	}
	if(attName[j]=='.')
...
Schema::Schema(const Schema &left, const Schema &right) : numAtts(left.numAtts + right.numAtts), myAtts(new Attribute[numAtts]), fileName(NULL)
{
